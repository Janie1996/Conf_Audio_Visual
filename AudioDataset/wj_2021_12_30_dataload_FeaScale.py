# -*- coding: UTF-8 -*-
"""
@file:wj_2021_12_30_dataload_FeaScale.py
@author: Wei Jie
@date: 2021/12/30
@description: 将分段好的feature加载到内存，归一化处理， 每次训练只需要检索
"""
import sys
import os
...
from FeatureExt import wj_2021_12_26_Scale as Scale
import pickle
...
def get_audio_split_withoutResize(allfeatures, framenum, winlen):
    ...
    for index in range(len(allfeatures)):
        ...
        while (idx < mfcc.shape[0] - winlen):
            ...
            if (sequence_idx == framenum):
                break
        features = Scale.scale_Mel(sequence_list)
...
if __name__ == '__main__':
    ...
    train_list, test_list = utils.randomSplit(length=len(name_emotionLabel), testNum=1160)

    train_loader1, test_loader1 = AudioLoadData('DATA/2021_12_28_name_fbank_dict_1.pickle', train_list,
                                                             test_list, name_emotionLabel, 32,
                                                             framelen=5, winlen=224)
