# -*- coding: UTF-8 -*-
"""
@file:wj_2022_01_04_proposed_Loss.py
@author: Wei Jie
@date: 2022/1/4
@description:只有原始音频数据特征，特征直接加载到内存中，使用提出的模型LSTM，监督两个结果
"""
...
parser.add_argument('--lr', type=float, default=0.008, help='learning rate, default=0.0002')
...
def train(train_loader,model,criterion,criterion1,optimizer,scheduler):
    ...
    for i, (video, index) in enumerate(train_loader):
        ...
        loss0 = criterion(pre,label)
        loss1 = criterion1(pre1,label)
        loss = loss0 + loss1
...
def main():
    ...
    for epoch in range(opt.niter):
        ...
        train(train_loader,model,criterion,criterion1,optimizer,scheduler)
