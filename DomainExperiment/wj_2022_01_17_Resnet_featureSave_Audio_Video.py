# -*- coding: UTF-8 -*-
"""
@file:wj_2022_01_17_Resnet_featureSave_Audio_Video.py
@author: Wei Jie
@date: 2022/1/17
@description:
"""
...
def train(train_loader_audio,train_loader_video,model,criterion,criterion_D,optimizer,scheduler):
    ...
    for i, data in enumerate(zip(train_loader_audio, train_loader_video)):
        ...
        if (len(data[0][0])):
            ...
            lossE_audio = criterion(pre, label)

        loss = lossE_audio # + 0.7* lossE_video +lossD

        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    scheduler.step()
    print('Train *Prec@Audio {:.3f};  {:.3f};  {:.3f} ; Prec@Domain {:.3f}; Loss {:.3f}  '.format(right/all,(right1+right2)/all,right2/all,right_d / all,allLoss / all))
    ...
    for epoch in range(opt.niter):
        ...
        if (acc > best_wa):
            ...
            torch.save(state_dict1,
                           "Checkpoint/Domain_40_AV_" + str(num) + "_Resnet.pth")
        if (acc1 > best_ua):
            best_ua = acc1
