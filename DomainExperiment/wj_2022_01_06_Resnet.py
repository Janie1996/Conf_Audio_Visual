...
from DomainModels import ResNetEncoder_DomainClassifer as Model
import pickle
...
def train(train_loader_audio,train_loader_video,model,criterion,criterion_D,optimizer,scheduler):
    ...
    for i, data in enumerate(zip(train_loader_audio, train_loader_video)):
        ...
        if(len(data[0][0])):
            ...
            label = data[0][1].cuda()
            pre, pre1, pre2, preD = model(audio)
        ...
        pre, pre1, pre2, preD = model(audio_video)
        ...
        lossD = criterion_D(preD, label_domain.unsqueeze(1))
        ...
        preds = torch.round(torch.sigmoid(preD)).squeeze(1)
...
def test(test_loader,model):
    ...
    with torch.no_grad():
        for i, (video, index) in enumerate(test_loader):
            ...
            pre, pre1, pre2,preD = model(video)
...
def main():
    ...
    model=Model.resnet18(classes=opt.nClasses)
