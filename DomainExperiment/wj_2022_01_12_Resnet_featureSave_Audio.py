# -*- coding: UTF-8 -*-
"""
@file:wj_2022_01_12_Resnet_featureSave_Audio.py
@author: Wei Jie
@date: 2022/1/17
@description:
"""
...
def train__(train_loader_audio,train_loader_video,model,criterion,criterion_D,optimizer,scheduler):
    model.train(True)
    right_d = 0
...
def train(train_loader_audio,train_loader_video,model,criterion,criterion_D,optimizer,scheduler):
    ...
    for i, data in enumerate(zip(train_loader_audio, train_loader_video)):
        ...
        if(len(data[0][0])):
            ...
            label = data[0][1].cuda()
            pre, dom, fea, emo_all = model(audio)
...
def main():
    ...
    for epoch in range(opt.niter):
        ...
        if (acc > best_wa):
            ...
            torch.save(state_dict1,
                           "Checkpoint/Domain_40_audio_" + str(num) + "_Resnet.pth")
        if (acc1 > best_ua):
            best_ua = acc1
    ...
    # if(best_wa>0.66):
    #     torch.save(state_dict1,
    #            "Checkpoint/Domain_40_" + str(best_wa) + "_Resnet.pth")  # , _use_new_zipfile_serialization=False)
