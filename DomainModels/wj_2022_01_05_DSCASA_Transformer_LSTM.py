# -*- coding: UTF-8 -*-
"""
@file:wj_2022_01_05_DSCASA_Transformer_LSTM.py
@author: Wei Jie
@date: 2022/1/5
@description: 传统CNN + 深度可分离卷积 + CASA + transformer + mean + LSTM +Domain Classifier
"""
...
class inno_model1(nn.Module):
    def __init__(self, input_channel, classes):
        ...
        self.featureDim = 2048#768   #2048 #8192
        ...
        self.temp_lstm1 = nn.LSTM(input_size=self.featureDim, hidden_size=128, num_layers=1, batch_first=True, bidirectional=False)
        self.temp_lstm2 = nn.LSTM(input_size=128, hidden_size=4, num_layers=1, batch_first=True, bidirectional=False)
    ...
    def forward(self, x):  # 16,5,3,224,40  b,l,c,frame,feature
        ...
        dom_out = self.domainFC(attn_out).view(-1,self.length)
        dom_out = torch.mean(dom_out,dim=1).unsqueeze(1)
        #  B,L,F
        cla_out = self.classifier(attn_out).view(-1,self.length,self.classes)
        cla_out = torch.transpose(cla_out,1,2)
        cla_out = self.score_avg(cla_out).squeeze(2)  #b,classes
        cla_out = F.softmax(cla_out,dim=1)

        attn_out = attn_out.view(self.real_batch, self.length, -1)

        out, _ = self.temp_lstm1(attn_out)
        out, _ = self.temp_lstm2(out)
        return out[:, -1, :], cla_out, F.softmax(out[:, -1, :],dim=1) + cla_out , dom_out
...
if __name__ == "__main__":
    ...
    # ca = SpatialAttention()
    # input = torch.rand(2,128,224,40)
    # out = ca(input)


    a = torch.rand(2,10, 3, 224, 128)
    model = inno_model1(input_channel=3,classes=4)
    x = model(a)
    print()
