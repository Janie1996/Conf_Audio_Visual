# -*- coding: UTF-8 -*-
"""
@file:wj_2022_01_07_DSC_RES_LSTM.py
@author: Wei Jie
@date: 2022/1/7
@description:
"""
...
from DomainModels import wj_2022_01_07_ResNet_Domain as DomainModel
from AudioModels import wj_2022_01_07_DSCASA_Transformer as AudioModel
from FeatureFusionModels import wj_2022_01_07_LSTM as FusionModel
import pickle
...
parser.add_argument('--featureDim',  default=128, type=int,help='the feature dimension of audio input')
...
def train(train_loader_audio,train_loader_video,model_D,criterion_D_E,criterion_D_D,optimizer_D,scheduler_D,model_A,criterion_A,optimizer_A,scheduler_A,model_F,criterion_F,optimizer_F,scheduler_F):
    ...
    for i, data in enumerate(zip(train_loader_audio, train_loader_video)):
        ...
        emo,dom,fea_d = model_D(audio_video)
    ...
    print('Train *Prec@Audio {:.3f};  {:.3f};  {:.3f} ; Prec@Domain {:.3f}; Loss {:.3f}  '.format(right/all_e,right1/all_e,right2/all_e,right_d / all,allLoss / all))
...
def test_UA_WA(test_loader,model_D,model_A,model_F):
    ...
    trace_y=[]
    trace_pre_y,trace_pre_y1,trace_pre_y2=[],[],[]
    ...
    with torch.no_grad():
        for i, (video, index) in enumerate(test_loader):
            ...
            pre,dom,fea_d = model_D(video)
            pre1,fea_a = model_A(video)
    ...
    return max(right/all,right1/all,right2/all),max(unweighted_accuracy,unweighted_accuracy1,unweighted_accuracy2)
...
def main():
    ...
    model_D = DomainModel.resnet18(classes=opt.nClasses)
    ...
    model_F = FusionModel.EmotionRecog(classes=opt.nClasses,featureDim=2048+512)
    ...
    scheduler_D = lr_scheduler.StepLR(optimizer_D, step_size=opt.step_size, gamma=opt.gamma)

    criterion_A = nn.CrossEntropyLoss()
    optimizer_A = torch.optim.SGD(filter(lambda p: p.requires_grad, model_A.parameters()), opt.lr,
                                  momentum=opt.momentum, weight_decay=opt.weight_decay)
    scheduler_A = lr_scheduler.StepLR(optimizer_A, step_size=opt.step_size, gamma=opt.gamma)

    criterion_F = nn.CrossEntropyLoss()
    optimizer_F = torch.optim.SGD(filter(lambda p: p.requires_grad, model_F.parameters()), opt.lr,
                                  momentum=opt.momentum, weight_decay=opt.weight_decay)
    scheduler_F = lr_scheduler.StepLR(optimizer_F, step_size=opt.step_size, gamma=opt.gamma)
