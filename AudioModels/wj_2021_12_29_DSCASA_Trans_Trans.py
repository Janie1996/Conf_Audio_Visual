# -*- coding: UTF-8 -*-
"""
@file:wj_2021_12_29_DSCASA_Trans_Trans.py
@author: Wei Jie
@date: 2021/12/29
@description:  传统CNN + 深度可分离卷积 + CASA + transformer + mean + transformer +FC
"""
import sys
import os
...
class inno_model1(nn.Module):
    def __init__(self, input_channel, classes):
        ...
        self.transf_encoder1 = nn.TransformerEncoder(transf_layer, num_layers=1)
    ...
    def forward(self, x):  # 16,5,3,224,40  b,l,c,frame,feature
        ...
        out = self.transf_encoder1(attn_out)
        out = torch.mean(out, dim=1)
        out = self.emotion(out)
        out = F.softmax(out,dim=1)

        # out, _ = self.temp_lstm1(attn_out)
        # out, _ = self.temp_lstm2(out)
        return out, cla_out, out+ cla_out
...
if __name__ == "__main__":
    ...
    a = torch.rand(2,10, 1, 224, 128)
    model = inno_model1(input_channel=1,classes=4)
    x = model(a)
    print()
